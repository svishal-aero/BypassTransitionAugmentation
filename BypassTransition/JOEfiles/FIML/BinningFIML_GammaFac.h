#ifndef FIML_AUG_GAMMAFAC_H
#define FIML_AUG_GAMMAFAC_H
...
class FIML_Aug_GammaFac
{
  ...
  public:
    ...
    FIML_Aug_GammaFac(void)
    {
      MPI_Comm_rank(MPI_COMM_WORLD, &rank);
      MPI_Comm_size(MPI_COMM_WORLD, &size);
      if(rank==0) printf("FIML_Aug_GammaFac()\n");
    }
    ...
    void init(std::string name_val)
    {
      ...
      features_AD.resize(nFeatures);
      dbeta_dftrs.resize(nFeatures);
    ...
    void evalFeatures(int icv)
    {
      ...
      features[1] = sqrt(nu_cv*omega_cv) / (sqrt(nu_cv*omega_cv) + sqrt(kine_cv/omega_cv*vort_cv));
      features[2] = d_cv*vort_cv / (d_cv*vort_cv + U_cv);
    ...
    void evalFeatures_AD(int icv)
    {
      ...
      features_AD[1] = sqrt(nu_cv*omega_cv) / (sqrt(nu_cv*omega_cv) + sqrt(kine_cv/omega_cv*vort_cv));
      features_AD[2] = d_cv*vort_cv / (d_cv*vort_cv + U_cv);
			//features_AD[2] = nu_cv / (nu_cv + kine_cv/omega_cv);
      
      for(int i=0; i<features.size(); i++) features[i] = features_AD[i].value();
    }
