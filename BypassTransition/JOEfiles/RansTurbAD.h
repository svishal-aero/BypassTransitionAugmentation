...
class RansTurb_AD : virtual public UgpWithCvCompFlow_AD, public RansTurb
{
  ...
  public:
    ...
    RansTurb_AD(void)
    {
      ...
      registerScalar(gamma_sep.psi, "gamma_sep_psi", CV_DATA);
      ...
      for(int i=0; i<nScal; i++)
      {
        if(strcmp(scalarTranspEqVector_AD[i].name,"gammai")==0)
        {
          for(int icv=0; icv<ncv_gg; icv++)
          {
            ...
            gamma_sep.psi[icv] = scalarTranspEqVector_psi[i].phi[icv];
          }
        }
      }
    }
    ...
    {
      adouble rhoVol, src, Prod, nuT, nuL, d_src; int noc00, index;
    
      for (int icv=0; icv<ncv_g; icv++)
      {
        rhoVol = rho_AD[icv] * cv_volume[icv];
        ...
        gamma_sep.calculate_AD(icv); adouble bfSep = getBlendingFactor_AD(icv);
        gamma_max.calculate_AD(icv); adouble bfMax = 1; //1 - bfSep;
        src   = (pow(gamma_max.beta_AD, bfMax) * pow(gamma_sep.beta_AD, bfSep) - gammai[icv])
    ...
    adouble sigmoid_AD(adouble x, adouble s)
    {
      return 1.0 / (1.0 + exp(-x/s));
    }

    adouble getBlendingFactor_AD(int icv)
    {
      ...
      for(int noc = noc_f + 1; noc <= noc_l; noc++)
      {
        ...
        adouble nbrVort = (icv_nbr >= ncv_gg) ? vortMag[icv] : vortMag[icv_nbr];
        FOR_I3 dvn = dvn + nbocv_all_grad_coeff[noc][i] * nbrVort * wn[i];
      }
      adouble vort_by_dist = vortMag[icv] / wallDist[icv];
      Param *swtch = getParam("SWITCH_AUG_AT");
      adouble result = dvn * vort_by_dist / (dvn * dvn + vort_by_dist * vort_by_dist);
      return sigmoid_AD(  result - swtch->getDouble(2), 0.003) *
             sigmoid_AD(- result + swtch->getDouble(1), 0.003);
    }
};
