...
#include <cstdlib>
#include <cmath>
#include "../GreenGaussLinAugMap.hpp"

int main()
{
  MPI_Init(NULL, NULL);
  
  GreenGaussLinAugMap<2> augMap;
  augMap.init("testMap", {10, 10}, {0, 0}, {1.000001, 1.000001});

  for(int i=0; i<10; i++) for(int j=0; j<10; j++)
  {
    augMap.params[i*10+j] = exp(-0.25*((i-3)*(i-3) + (j-8)*(j-8)));
  }
