  ...
  public:
    ...
    )
    {
      ...
      for(int icv=0; icv<ncv; icv++)
      {
        ...
        src   = (gamma_max.beta - gammai[icv]) * vortMag[icv] * sqrt(gammai[icv]);
        d_src = -1.5 * vortMag[icv] * sqrt(gammai[icv]);
        gamma_max.dR_dBeta[icv] = vortMag[icv] * sqrt(gammai[icv]);
    ...
    void writeDerivsFIML(void) { gamma_max.writeSens(); }
